...
case $NOTIFICATION in
	...
	all) slackNotify "${JOBNAME}: $(getResultText $scriptExitCode) Log: ${my_log_link}";;
	failure) if [ "${scriptExitCode}" != "0" ]; then slackNotify "${JOBNAME}: $(getResultText $scriptExitCode) Log: ${my_log_link}"; fi;;
esac
